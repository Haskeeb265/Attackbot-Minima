"""
Thin, robust wrapper around ``docker run`` for every containerised recon tool.

Extracted from the names pipeline's passive stage into the platform's
``common/`` package so the siblings share one implementation instead of
...
log = logging.getLogger("platform.common.docker_tool")
