...
```bash
python -m service.recon_pipeline.asset_pipelines.subdomain_domain_wildcards.passive.pipeline
```
...
1. **Leakage check** — `foreign.txt` should be empty or hold a handful of
   ...
   ```bash
   wc -l service/recon_pipeline/asset_pipelines/subdomain_domain_wildcards/passive/output/foreign.txt
   ```
